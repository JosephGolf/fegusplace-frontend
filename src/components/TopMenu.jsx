...
const TopMenu = (props) => {
 ...
 const [products, setProducts] = React.useState([]);
  ...
  return (
    <nav id="vertical-menu" style={{ fontFamily: 'ABeeZee' }}>
      ...
      <ul className="main-menu">
        {props.data.map((item, index) => (
          ...
          >
            <Link to={`/category/${item.nameEn}`} style={{ background: '#bebebf', color: 'black', textDecoration: 'none' }}>
                      {t(item.nameEn)}
                    </Link>
                    ...
                    <ul className="submenu-2">
                      {sub.subCatArray &&
                        sub.subCatArray.map((arr, index) => (
                          ...
                          >
                            <Link
                                to={`/category/${item.nameEn}/${sub.subCatName}/${arr}`}
                                style={{ background: '#000080', color: 'white', textDecoration: 'none' }}
                            >{arr}</Link>
                          </li>
                        ))}
                    </ul>
